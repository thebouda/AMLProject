...
In addition, a variant of the Federated Average algorithm with server momentum was also considered, called FedAvgM \ref{DBLP:journals/corr/abs-1909-06335}. Vanilla FedAvg updates the weights via (FORMULA). To add momentum at the server side, a different computation is made instead (FORMULA) and update the model with (FORMULA). 
...
% three performed better with the Delta variable activated, and one specifically (8-2) outperformed every over scenario with an accuracy of 78,27\%. The accuracies presented in the figure \ref{AccDiff} are very similar between each other, even though with the deactivation of the Delta variable (each client with the same amount of data) it seems to perform better, as 5/8 tests had greater accuracy than by using Delta.

% (CHECK THAT AGAIN, THIS COULD BE MISLEADING)
% However, computing the mean of the performances, we get 72,1 \% and 72,8\% for no Delta and Delta respectively. This translates to the fact that even though the Delta variable presented better accuracy, the accuracy achieved with the Delta activated is greater. This result could be explained by the fact that adding some randomness to the data quantities helps the prediction of classes.
...
% \begin{figure}
%     \centering
%     \includegraphics[width=0.5\textwidth,height=.2\textheight]{1_DirichletDistr.png}
%     \caption{Impact of Dirichlet on data distribution from (REFERENCE)}
%      \label{DirichletDistr} 
% \end{figure}
...
% \begin{figure}
%     \centering
%     \includegraphics[width=0.5\textwidth,height=.3\textheight]{8_FedAvgM_from_paper.png}
%     \caption{FedAvg vs FedAvgM behaviour from paper (REFERENCE)}
%      \label{FedAvgMpaper} 
% \end{figure}
...
The general idea that can be drawn from this work is that, although it doesn't have the accuracy of the centralised model, Federated Learning works and can be used in real life scenarios.
